...
$data.getODataServerMiddleware = function(type, db) {
    var connect = require('connect');
    var basicAuth = require('basic-auth-connect');
    var domain = require('domain');
   ...
   var errorHandlerFn = function(err, req, res, next){
	...
	return {
	    ...
	    afterBasicAuth: function(req, res, next) {
			  console.log('afterBasicAuth begin');
	    	  config.provider.user = config.user = req.user || req.remoteUser || config.user || config.provider.user || 'anonymous';
	    	  next();
	    },
	    ...
	    simpleBody: function(req, res, next){
			  console.log('simpleBody begin');
	        $data.JayService.OData.Utils.simpleBodyReader()(req, res, next);
	    },
	    error: function(req, res, next, callback){
